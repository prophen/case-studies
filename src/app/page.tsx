...
export default function Home() {
  ...
  return <main id="main">
    ...
    <section id="work" className="work-section"><div className="section-label"><h2>01 / Selected work</h2><span>A closer look at the process.</span></div>
      {studies.length ? [...studies].sort((a,b) => Number(b.featured) - Number(a.featured)).map(study => <article className="feature" key={study.slug}><div className="feature-copy"><p className="eyebrow">{study.project}</p><h3>{study.title}</h3><p className="feature-summary">{study.summary}</p>{study.draft && <span className="draft-badge">Draft case study</span>}<Link className="text-link" href={`/case-studies/${study.slug}`}>Open the field notes <span aria-hidden="true">↗</span></Link></div>{study.cover ? <Image className="feature-cover" src={study.cover} alt={study.coverAlt!} width={study.coverWidth!} height={study.coverHeight!} sizes="(max-width: 800px) 88vw, 65vw"/> : study.draft && study.slug === 'social-content-agent' ? <WorkflowPreview/> : <p className="handwriting">A story taking shape.</p>}</article>) : <div className="empty-work"><h3>The field notes are taking shape.</h3><p>For now, you can find my writing on <a href="https://nikema.dev">nikema.dev ↗</a>.</p></div>}
    </section>
